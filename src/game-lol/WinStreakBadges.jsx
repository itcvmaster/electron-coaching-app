    ...
    >
      ...
      {showTooltip && (
        <div className={TooltipContainerWithArrow}>
          <p className={"championwinrate"}>
            {queueStr}{" "}
            {t("lol:streak", "{{streak}} Streak:", {
              streak: streakType === "win" ? "Win" : "Losing",
            })}
          </p>
          <p className={"championname"} style={{ textAlign: "center" }}>
            {streakCount}
          </p>
        </div>
      )}
...
const LoLBadge = ({ styles, queue, hideAnimation = false, region, name }) => {
  const state = useSnapshot(readState);
  const { t } = useTranslation();
  const account = state?.lol?.profiles?.[getDerivedId(region, name)];
  const accountError = account instanceof Error ? account : null;
  const queueId = 420;
  const queueStr = translateQueueType(t, queue || queueId);
  ...
  if (!account || !matchList || accountError || matchListError) return null;

  const calcWinStreak = () => {
    if (!account || !matchList) return { fCount: 0, fMode: 0 };
    let mode = -1;
    let count = 0;
    for (const matchlistItem of matchList) {
      const match = state.lol?.matches?.[matchlistItem.id];
      if (!match) continue;
      const { gameDuration, participants } = match;
      if (!gameDuration || gameDuration < 300 || !participants) continue;
      const myPlayer = participants.find((p) => p?.summonerName === name);
      if (!myPlayer) return { fCount: 0, fMode: mode };
      const win = myPlayer.win;

      // TODO(S): fix this stupid
      if (mode === -1) mode = win ? 1 : 0;
      if ((mode === 1 && !win) || (mode === 0 && win)) break;
      count++;
    }
    return { fCount: count, fMode: mode };
  };

  const { fCount, fMode } = calcWinStreak();

  return (
    <>
      <Badge
        styles={styles}
        streakType={fMode === 1 ? "win" : "loss"}
        streakCount={fCount}
        queueStr={queueStr}
        ...
        className={hideAnimation ? (fMode === 1 ? " win" : " loss") : ""}
      />
    </>
  );
};
